...
function score( dice ) {
    ...
    let rolled = [0, 0, 0, 0, 0, 0]        
    
    dice.forEach(die => rolled[die-1]++)   
    
    rolled.forEach((count, dieNumber) => {
      if (dieNumber === 0) {
        if (count >= 3) score = 1000 + ((count-3) * 100)
        if (count < 3) score = count * 100
      }
      
      if (dieNumber === 4) {
        if (count >= 3) score += 500 + ((count-3) * 50)
        if (count < 3) score += count * 50
      }
      
      if (dieNumber != 0 && dieNumber != 4) {
        if (count >= 3) score += (dieNumber + 1) * 100  
      }
    })
