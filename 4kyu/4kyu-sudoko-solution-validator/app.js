...
function validSolution(board){
    ...
    function check(arr) {
      for (const set of arr) {
        let setCopy = set
        while (setCopy.length) {
          const checkNum = setCopy.splice(0,1)[0]        
          if (setCopy.includes(checkNum)) return false
        }
      }
      return true
    }
    ...
    if (check(rows) && check(columns) && check(squares)) return true
    return false
  }